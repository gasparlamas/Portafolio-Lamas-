    ...
    public static <T extends IGrafoDirigido> T cargarGrafo(String nomArchVert, String nomArchAdy, 
        ...
        for (String aristaString : aristas) {
            if ((aristaString != null) && (!aristaString.trim().isEmpty())) {
                ...
                aristasList.add(new TArista(datos[0], datos[1], Integer.parseInt(datos[2])));
            }
        }
        T elGrafo = null;
        try {
            elGrafo = (T) t.getDeclaredConstructor(Collection.class, Collection.class).newInstance(verticesList, aristasList);
        } catch (Exception ex) {
