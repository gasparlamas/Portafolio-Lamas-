...
public class TVertice<T> implements IVertice {
	...
	public void bpf(Collection visitados) {
		visitados.add(this);
		for (IAdyacencia vertice : adyacentes) {
			if (!visitados.contains(vertice.getDestino())) {
				vertice.getDestino().bpf(visitados);
			}
		}
	}
	...
	@Override
	public void obtenerCaminos(IVertice destino, HashSet<IVertice> visitados,
			LinkedList<TCamino> caminos,
			TCamino camino) {
		camino.add(this);
		if (this.getEtiqueta().equals(destino.getEtiqueta())) {
			...
			for (IAdyacencia adyacencia : this.getAdyacentes()) {
				...
				if (!visitados.contains(w)) {
					w.obtenerCaminos(destino, visitados, caminos, camino);
				}
			}
			visitados.remove(this);
		}
		camino.remove(this);
	}
}
